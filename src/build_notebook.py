"""Phase 8: assemble notebooks/CLO4_IDS_ML_Solution.ipynb (13 sections, objective.md Section 7.A) and execute it.

Narrative numbers are read from artifacts/ at build time; every table and figure inside the notebook
is recomputed by the notebook's own cells when it executes.
"""
import json
import sys
import time
...
`artifacts/realtime_report.json`; Section 12 re-measures live, and wall-clock timings vary by a few µs between runs.)"""),
        ...
        md("## 2. Library Imports & Seed Initialisation"),
        code("""import sys, time, json, warnings
...
the empirical proof for one downstream engineering decision."""),
        code("""display(Image(filename=eda.fig_attack_distribution(df), width=1000))"""),
...
records fabricates physically impossible packets."""),
        code("""display(Image(filename=eda.fig_volume_skew(df, rep), width=1000))
...
spikes)."""),
        code("""display(Image(filename=eda.fig_top_protocols(df), width=1000))"""),
...
on them: `OneHotEncoder(handle_unknown='ignore')`."""),
        code("""corr_path, top_pairs, label_corr = eda.fig_correlation(df)
...
print(f"champion fitted in {time.perf_counter() - t0:.1f}s  |  out-of-bag accuracy {rf.oob_score_:.4f}")"""),
        md(f"""**Why the ≥ 95 % gate is not fully reachable (evidence, not excuse).** A separate experiment
...
display(Image(filename=realtime.fig_latency(tab, per_call, all_cores), width=1000))"""),
        code("""fam, fam_rep = realtime.train_family_classifier(d["pre"], X_tr_raw, cat_tr_raw, X_te_raw, cat_te_raw)
...
(e.g. Treelite/ONNX) for the inline path."""),
        ...
        code("""print(f"Notebook executed end-to-end in {time.perf_counter() - T_START:.1f} s")"""),
    ]
...
def main():
    ...
    if errors:
        sys.exit(f"{len(errors)} cell(s) raised errors")
